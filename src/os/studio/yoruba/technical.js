// ============================================================
// FORGE YORUBA TECHNICAL REGISTRY — ENGLISH RETAINED  (Alpha 1.2)
// Same shape as ../hausa/technical.js — see that file's header.
// ============================================================
...
const tech = (english, { candidate = null, source = SOURCES.NONE,
  ...
  Object.freeze({ english, yoruba: candidate, retainEnglish: true, category: "technical", source, confidence, approved: false, usage, note });
...
export function technicalTerm(english) {
  ...
  if (!t) return english;
  if (t.retainEnglish || !t.approved || !t.yoruba) return english;
  if (t.confidence === CONFIDENCE.NATIVE_REVIEW_REQUIRED) return english;
  return t.yoruba;
}

export const candidatesAwaitingReview = () =>
  Object.freeze(TECHNICAL.filter((t) => t.yoruba && !t.approved)
    .map((t) => Object.freeze({ english: t.english, candidate: t.yoruba, confidence: t.confidence, note: t.note })));

export const termsWithNoCandidate = () => Object.freeze(TECHNICAL.filter((t) => !t.yoruba).map((t) => t.english));
