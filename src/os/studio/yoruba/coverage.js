// ============================================================
// FORGE YORUBA COVERAGE — COUNTED, NOT CLAIMED  (Alpha 1.2)
...
export function supportStatement(lang = "yo") {
  const ui = i18nCoverage(lang);
  const pack = lang === "yo" ? lexiconCoverage() : null;
  const parts = [
  ...
  if (pack) {
    parts.push(
      pack.approvedForProduction === 0
        ? `The Forge Yoruba lexicon has ${pack.totalEntries} researched entries and NONE ` +
          `approved for production — ${pack.webSourced} are web-sourced and awaiting native review.`
        : `${pack.approvedForProduction} of ${pack.totalEntries} Forge Yoruba entries are approved.`,
    );
    parts.push(
      `Forge AI cannot yet answer in Yoruba: ${pack.questionPatternsApproved} of ` +
...
export const translatorBrief = () => Object.freeze({
  ui: i18nCoverage("yo"), pack: lexiconCoverage(),
  technicalTermsNeedingTranslation: Object.freeze(TECHNICAL.filter((t) => !t.yoruba).map((t) => t.english)),
