// ============================================================
// FORGE IGBO COVERAGE — COUNTED, NOT CLAIMED  (Alpha 1.2)
...
export function supportStatement(lang = "ig") {
  const ui = i18nCoverage(lang);
  const pack = lang === "ig" ? lexiconCoverage() : null;
  const parts = [
  ...
  if (pack) {
    parts.push(
      pack.approvedForProduction === 0
        ? `The Forge Igbo lexicon has ${pack.totalEntries} researched entries and NONE ` +
          `approved for production — ${pack.webSourced} are web-sourced and awaiting native review.`
        : `${pack.approvedForProduction} of ${pack.totalEntries} Forge Igbo entries are approved.`,
    );
    parts.push(
      `Forge AI cannot yet answer in Igbo: ${pack.questionPatternsApproved} of ` +
...
export const translatorBrief = () => Object.freeze({
  ui: i18nCoverage("ig"), pack: lexiconCoverage(),
  technicalTermsNeedingTranslation: Object.freeze(TECHNICAL.filter((t) => !t.igbo).map((t) => t.english)),
