// ============================================================
// FORGE HAUSA TECHNICAL REGISTRY — ENGLISH RETAINED  (Alpha 1.2)
//
// Same shape as ../urhobo/technical.js. This pass's research (two general
// word lists, see lexicon.js) had no reason to surface ElectionCanon-
// specific civic-tech vocabulary, so EVERY term here retains English —
// that is an honest "not yet researched," not a claim that no Hausa
// equivalent could ever be found.
// ============================================================
...
const tech = (english, { candidate = null, source = SOURCES.NONE,
  ...
  Object.freeze({ english, hausa: candidate, retainEnglish: true, category: "technical", source, confidence, approved: false, usage, note });
...
export function technicalTerm(english) {
  ...
  if (!t) return english;
  if (t.retainEnglish || !t.approved || !t.hausa) return english;
  if (t.confidence === CONFIDENCE.NATIVE_REVIEW_REQUIRED) return english;
  return t.hausa;
}

export const candidatesAwaitingReview = () =>
  Object.freeze(TECHNICAL.filter((t) => t.hausa && !t.approved)
    .map((t) => Object.freeze({ english: t.english, candidate: t.hausa, confidence: t.confidence, note: t.note })));

export const termsWithNoCandidate = () => Object.freeze(TECHNICAL.filter((t) => !t.hausa).map((t) => t.english));
