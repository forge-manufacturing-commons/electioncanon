// ============================================================
// FORGE PIDGIN COVERAGE — COUNTED, NOT CLAIMED  (Alpha 1.2)
// Same rule as ../hausa/coverage.js.
// ============================================================
...
export function lexiconCoverage() {
  ...
  return Object.freeze({
    ...
    commonUsageUncorroborated: by(CONFIDENCE.COMMON_USAGE),
...
export function supportStatement(lang = "pcm") {
  const ui = i18nCoverage(lang);
  const pack = lang === "pcm" ? lexiconCoverage() : null;
  const parts = [
  ...
  if (pack) {
    parts.push(
      pack.approvedForProduction === 0
        ? `The Forge Pidgin lexicon has ${pack.totalEntries} researched entries and NONE ` +
          `approved for production — ${pack.webSourced} are web-sourced, ${pack.commonUsageUncorroborated} ` +
          `are common-usage-but-uncorroborated, all awaiting native review.`
        : `${pack.approvedForProduction} of ${pack.totalEntries} Forge Pidgin entries are approved.`,
    );
    parts.push(
      `Forge AI cannot yet answer in Pidgin: ${pack.questionPatternsApproved} of ` +
...
export const translatorBrief = () => Object.freeze({
  ui: i18nCoverage("pcm"), pack: lexiconCoverage(),
  technicalTermsNeedingTranslation: Object.freeze(TECHNICAL.filter((t) => !t.pidgin).map((t) => t.english)),
